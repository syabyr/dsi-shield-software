...
static const uint8_t edid_psv1000[] =
#include "edid_psv1000.h"
;
...
static const struct pll_config pll_psv1000 = {
    26, 7, 1, 0,
    pll_regs
};


static int psv1000_init(struct dsi_panel_config *panel)
{
    //MCS Access Password
    const unsigned char mcspwd[] = { 0xF0,0x5A,0x5A };
    dsi_long_write(0, mcspwd, sizeof(mcspwd));

    const unsigned char mcspwd1[] = { 0xF1,0x5A,0x5A };
    dsi_long_write(0, mcspwd1, sizeof(mcspwd1));

    const unsigned char init1[] = { 0xB0,2 };
    dsi_long_write(0, init1, sizeof(init1));

    const unsigned char init2[] = { 0xF3,0x3B };
    dsi_long_write(0, init2, sizeof(init2));
    
    const unsigned char init3[] = { 0xF4,0x33,0x42,0,8 };
    dsi_long_write(0, init3, sizeof(init3));
    
    const unsigned char init4[] = { 0xF5,0,6,0x26,0x35,3 };
    dsi_long_write(0, init4, sizeof(init4));

    const unsigned char init5[] = { 0xF6,2 };
    dsi_long_write(0, init5, sizeof(init5));

    const unsigned char init6[] = { 0xC6,0xB,0,0,0x3C,0,0x22,0,0,0,0};
    dsi_long_write(0, init6, sizeof(init6));

    const unsigned char init7[] = { 0xF7,0x20 };
    dsi_long_write(0, init7, sizeof(init7));

    //
    const unsigned char init31[] = { 0xF5,0,6,0x27,0x35,3 };
    dsi_long_write(0, init31, sizeof(init31));

    const unsigned char init32[] = { 0xB2,6,6,6,6};
    dsi_long_write(0, init32, sizeof(init32));

    const unsigned char init33[] = { 0xB1,7,0,0x10 };
    dsi_long_write(0, init33, sizeof(init33));

    const unsigned char init34[] = { 0xF8,0x7F, 0x7A, 0x89, 0x67, 0x26, 0x38, 0, 0, 9, 0x67, 0x70, 0x88, 0x7A, 0x76, 5, 9, 0x23, 0x23, 0x23};
    dsi_long_write(0, init34, sizeof(init34));
...
struct dsi_panel_config panel_psv1000 = {
    "AMS495QA04 (PSV1000)",
    2, /* num_lanes */
